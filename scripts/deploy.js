  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      shell.exec(cmd, (err, res, stderr) => {
        ...
        try {
          const { updates = {} } =
            JSON.parse(convert.xml2json(res, { compact: true, spaces: 2 })) ||
            {};
...
function cpDepositeFile() {
  return new Promise((resolve, reject) => {
    try {
      ...
      const updateXmlPath = path.resolve(
        __dirname,
        `../auto_update/public/${updateXmlName}.xml`,
      );
      const manifestPath = path.resolve(__dirname, `../manifest.json`);
      const crxFilePath = path.resolve(
        __dirname,
        `../build/${version}/${zipName}.crx`,
      );
      const zipFilePath = path.resolve(
        __dirname,
        `../build/${version}/${zipName}.zip`,
      );
      fs.writeFileSync(updateXmlPath, xmlString);
      fs.copyFileSync(
        manifestPath,
        path.resolve(__dirname, "../auto_update/public/manifest.json"),
      );
      fs.copyFileSync(
        crxFilePath,
        path.resolve(__dirname, `../auto_update/public/${latestZipName}.crx`),
      );
      fs.copyFileSync(
        zipFilePath,
        path.resolve(__dirname, `../auto_update/public/${latestZipName}.zip`),
      );
      resolve(true);
    } catch (e) {
      console.error(chalk.red(`\n[发布]: copy资料发生错误`));
...
function saveDepositeFile() {
  try {
    ...
    return git
      .add("./")
      .commit(`[auto_update]发布 ${name} ${version}`)
      .push("origin", "master");
  } catch (e) {
    console.error(chalk.red(`\n[发布]: 推送更新到托管服务失败`));
    throw e;
